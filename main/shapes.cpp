...
std::vector<Vec2f> texcoords;
  ...
  for (std::size_t i = 0; i < aSubdivs; ++i) {
    ...
    if (aCapped) {
      ...
      Vec3f normalCap = normalize(cross(Vec3f{0.f, 0.f, 1.f}, vertex3 - vertex1));
  ...
  return SimpleMeshData{std::move(pos), std::move(col), std::move(normals), std::move(texcoords)};
}
...
std::vector<Vec2f> texcoords;
  ...
  for (std::size_t i = 0; i < aSubdivs; ++i) {
    ...
    if (aCapped) {
      ...
      // Normals for capped triangles
      Vec3f normalCap = normalize(cross(Vec3f{0.f, 0.f, 1.f}, vertex4 - vertex2));
...
SimpleMeshData make_prism(bool aCapped, std::size_t aSubdivs, Vec3f aColor,
  ...
  for (std::size_t i = 0; i < aSubdivs; ++i) {
    float const angle = i / static_cast<float>(aSubdivs) * 2.f * pi;
    float const x = std::cos(angle);
    float const y = std::sin(angle);

    Vec3f vertex1{x, y, 0.f};
    Vec3f vertex2{x, y, 1.f};

    // Calculate normals for each triangle
    Vec3f normal = normalize(cross(vertex2 - vertex1, Vec3f{0.f, 0.f, 1.f}));

    // Two vertices create one segment of the prism’s shell.
    pos.push_back(vertex1);
    pos.push_back(vertex2);
    ...
    if (aCapped) {
      // Add caps
      Vec3f cap1{0.f, 0.f, 0.f};
      Vec3f cap2{0.f, 0.f, 1.f};
      pos.push_back(cap1);
      pos.push_back(cap2);

      Vec3f normalCap = normalize(cross(Vec3f{0.f, 0.f, 1.f}, vertex2 - vertex1));
  ...
  // Transform normals and positions
  for (auto& normal : normals) {
    normal = normalize(N * normal);
  }
